...
public class UserService {
	...
	public String findAllByPage(String name,String date_from,String date_to,String sortName,String order,int first, int pageSize) {
		...
		//鎷煎啓SQL璇彞
		StringBuffer sqlBuffer = new StringBuffer("SELECT id,name,email,regdate,authority FROM users WHERE 1=1");
		//鍒ゆ柇name鏄惁瀛樺湪
		if(!(name == null || name.length() <= 0)){
			//濡傛灉瀛樺湪鍚慡tringBuffer涓拷鍔爏ql璇彞
			sqlBuffer.append(" AND NAME LIKE '%"+name+"%'");
	...
	public int addUser(User u) {
		String sql ="INSERT INTO users(NAME, password ,email, authority, regdate) VALUES ('"
						+u.getName()+"', '"+u.getPassword()+"', '"+u.getEmail()+"', '"+u.getAuthority()+"', '"+u.getRegdate()+"');";
		System.out.println(sql);
		int rows = SqlHelper.executeUpdate(sql, null);
		return rows;
	}
	...
	public int updateUser(User u) {
		...
		String sql = "UPDATE users SET password = '"+u.getPassword()+"', authority = '"+u.getAuthority() +"', email = '"+u.getEmail()+"' WHERE id = "+u.getId()+";" ;
		System.out.println(sql);
		int rows = SqlHelper.executeUpdate(sql, null);
		return rows;
	}
	...
	public List<User> checkLogin(String manager, String password) {
		String sql = "SELECT id,name,email,regdate,authority FROM users WHERE name ='"+manager+"' AND password = '"+password+"';";
		//System.out.println(sql);
		ResultSet rs = SqlHelper.executeQuery(sql, null);
		List<User> users = new ArrayList<User>();
		try {
	...
	public User findUserById(String id) {
		String sql = "SELECT id,name,password,email,regdate,authority FROM users WHERE id = "+id+";";
